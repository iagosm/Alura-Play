...
class VideoRepository
{
    ...
    public function removeImagePath($id): bool
    {
        $this->deletePathPast($id);
    ...
    public function deletePathPast($id) 
    {
        $result = false;
        $sql = "SELECT image_path FROM videos WHERE id = ?";
        $statement = $this->pdo->prepare($sql);
        $statement->bindValue(1, $id);
        $statement->execute();
        $pathImage = __DIR__ . '/../../public/img/uploads/' . $statement->fetch()['image_path'];
